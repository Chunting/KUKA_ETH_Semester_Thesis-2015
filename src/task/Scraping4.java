...
* @see #dispose()
*/
public class Scraping4 extends RoboticsAPIApplication {
	...
	private static final double v_transition_impact = 50 ; // velocity during transition phase for impact[mm/s]
	...
	private static final double contact_dx = 250;
	...
	public void run() {
		...
		int direction = getApplicationUI().displayModalDialog( ApplicationDialogType.QUESTION, "What do you want to do ?", "Scrape", "Get data", "BE AWESOME" );
		...
		F_contactStart 	= new Frame(lbrRoot,410					,0, 80,Math.toRadians(0),Math.toRadians(30),Math.toRadians(180));
		F_contactEnd 	= new Frame(lbrRoot,410+contact_dx		,0, 70,Math.toRadians(0),Math.toRadians(30),Math.toRadians(180));
		F_contactEndup  = new Frame(lbrRoot,410+contact_dx		,0, 80,Math.toRadians(0),Math.toRadians(30),Math.toRadians(180));
		F_flightmid     = new Frame(lbrRoot,410+(0.5*contact_dx),0,100,Math.toRadians(0),Math.toRadians(20),Math.toRadians(180));
    ...
    private void filesend(int dir,int i_cycle, int imp) {
	    ...
	    String destinationPath_PC = "C:/Users/RoboCut/Desktop/experiments/scrape" + dat + cycle + impact ;
